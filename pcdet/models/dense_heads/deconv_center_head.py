...
class SeparateHead(nn.Module):
    def __init__(self, input_channels, sep_head_dict, stride=1, head_conv=64, init_bias=-2.19, use_bias=False, norm_func=None):
        super(SeparateHead, self).__init__()
        if stride > 1:
            self.deblock = nn.Sequential(
                nn.ConvTranspose2d(input_channels, head_conv, kernel_size=int(stride),
                                   stride=int(stride), padding=0),
                nn.BatchNorm2d(head_conv),
                nn.ReLU()
            )
            input_channels = head_conv
        else:
        ...
        for cur_name in self.sep_head_dict:
            ...
            for k in range(num_conv - 1):
                fc_list.append(nn.Sequential(
                    nn.Conv2d(input_channels, head_conv, kernel_size=3, stride=1, padding=1, bias=use_bias),
                    nn.BatchNorm2d(head_conv) if norm_func is None else norm_func(input_channels),
                    nn.ReLU()
                ))
            fc_list.append(nn.Conv2d(head_conv, output_channels, kernel_size=3, stride=1, padding=1, bias=True))
...
class DeConvCenterHead(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
                 predict_boxes_when_training=True):
        super(DeConvCenterHead, self).__init__()
        ...
        for (cur_class_names, stride) in zip(self.class_names_each_head, strides):
    ...
    def assign_target_of_single_head(
        ...
        """
        feature_map_size = torch.Size((feature_map_size[0] * 2, feature_map_size[1] * 2))
    ...
    def fuse_deconv_and_bn(self, deconv, bn):
        fused_deconv = nn.ConvTranspose2d(in_channels=deconv.in_channels,
        ...
        w_bn = torch.diag(bn.weight.div(torch.sqrt(bn.eps + bn.running_var)))
        fused_deconv.weight.copy_(torch.mm(w_bn, w_conv).view(fused_deconv.weight.shape))

        b_conv = torch.zeros(deconv.weight.size(0), device=deconv.weight.device) if deconv.bias is None else deconv.bias
        b_bn = bn.bias - bn.weight.mul(bn.running_mean).div(torch.sqrt(bn.running_var + bn.eps))
        fused_deconv.bias.copy_(torch.mm(w_bn, b_conv.reshape(-1, 1)).reshape(-1) + b_bn)
        return fused_deconv

    def fuse(self):
        print("fusing...")
        conv = self.shared_conv[0]
        bn = self.shared_conv[1]
        act = self.shared_conv[2]
        fused_conv = self.fuse_conv_and_bn(conv, bn)
        self.shared_conv = nn.Sequential(
            fused_conv,
            act
        )
        ...
        for head in self.heads_list:
            deblock = head.__getattr__("deblock")
            if not isinstance(deblock, nn.Identity):
                convTranspose = deblock[0]
                bn = deblock[1]
                act = deblock[2]
                fused_convTranspose = self.fuse_deconv_and_bn(convTranspose, bn)
                deblock = nn.Sequential(
                    fused_convTranspose,
                    act
                )
            head.__setattr__("deblock", deblock)
            for block_str in ['center', 'center_z', 'dim', 'rot', 'hm']:
                block = head.__getattr__(block_str)
                conv1 = block[0][0]
                bn = block[0][1]
                act = block[0][2]
                conv2 = block[1]
                fused_conv1 = self.fuse_conv_and_bn(conv1, bn)
                head.__setattr__(block_str, nn.Sequential(
                    nn.Sequential(
                        fused_conv1,
                        act
                    ),
