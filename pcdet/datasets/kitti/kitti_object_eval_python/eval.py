...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    if len(current_classes) > 1:
        result += print_str(
            ("Overall AP@{}, {}, {}:".format(*difficulty))
        )
        mAPbbox_mean = mAPbbox.mean(axis=0)
        result += print_str(
            "bbox AP:{:.4f}, {:.4f}, {:.4f}".format(*mAPbbox_mean[:, 0])
        )
        mAPbev_mean = mAPbev.mean(axis=0)
        result += print_str(
            "bev  AP:{:.4f}, {:.4f}, {:.4f}".format(*mAPbev_mean[:, 0])
        )
        mAP3d_mean = mAP3d.mean(axis=0)
        result += print_str(
            "3d   AP:{:.4f}, {:.4f}, {:.4f}".format(*mAP3d_mean[:, 0])
        )
        mAPaos_mean = mAPaos.mean(axis=0)
        result += print_str(
            "aos  AP:{:.2f}, {:.2f}, {:.2f}".format(*mAPaos_mean[:, 0])
        )

        result += print_str(
            ("Overall AP_R40@{}, {}, {}:".format(*difficulty))
        )
        mAPbbox_R40_mean = mAPbbox_R40.mean(axis=0)
        result += print_str(
            "bbox AP:{:.4f}, {:.4f}, {:.4f}".format(*mAPbbox_R40_mean[:, 0])
        )
        mAPbev_R40_mean = mAPbev_R40.mean(axis=0)
        result += print_str(
            "bev  AP:{:.4f}, {:.4f}, {:.4f}".format(*mAPbev_R40_mean[:, 0])
        )
        mAP3d_R40_mean = mAP3d_R40.mean(axis=0)
        result += print_str(
            "3d   AP:{:.4f}, {:.4f}, {:.4f}".format(*mAP3d_R40_mean[:, 0])
        )
        mAPaos_R40_mean = mAPaos_R40.mean(axis=0)
        result += print_str(
            "aos  AP:{:.2f}, {:.2f}, {:.2f}".format(*mAPaos_R40_mean[:, 0])
        )
